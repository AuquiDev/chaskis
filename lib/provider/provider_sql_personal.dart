...
import 'package:chaskis/sqllite/db_crud_personal.dart';
import 'package:chaskis/utils/custom_text.dart';

 class DBPersonalProvider with ChangeNotifier {
  List<TPersonalModel> listsql = []; //LISTA para alamcenar los datos

  List<TPersonalModel> get data => listsql;
//Intancia del Crud de asistencai DB local.
  CrudDBPersonal crudDb = CrudDBPersonal();
  ...
  Future<void> insertData(TPersonalModel e) async {
    isSyncing = true;
    notifyListeners();
  ...
  Future<void> updateData(TPersonalModel e, int? idsql) async {
    isSyncing = true;
    notifyListeners();
...
//METODO GUARDAR EN SQLITE
  ...
  Future<void> guardarEnSQlLite( List<TPersonalModel> api, BuildContext context) async {
    incidenciasCarga = [];
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Column(
                  ...
                  children: [
                    const H2Text(
                      text: 'Hemos detectado que el Personal:',
                      fontSize: 12,
